...
public final class ReviewsPanel extends JPanel {
    ...
    private JComponent createAddReviewCard() {
        ...
        JLabel sub = label(
                "Tell us how your rental experience went.",
                12,
                Font.PLAIN,
                MUTED
        );
        ...
        vehicleBox = new DarkComboBox<>(
                new String[]{"No rented vehicles available"}
        );
    ...
    private JComponent createReviewsCard() {
        ...
        reviewCountLabel = label(
                "0 reviews",
                11,
                Font.BOLD,
                PALE
        );
        reviewCountLabel.setHorizontalAlignment(
        ...
        reviewsList.setLayout(
                new BoxLayout(reviewsList, BoxLayout.Y_AXIS)
        );
    ...
    private JComponent createActivityCard() {
        ...
        JLabel sub = label(
                "Your contribution to the Velora community",
                12,
                Font.PLAIN,
                MUTED
        );
        ...
        titleText.setLayout(
                new BoxLayout(titleText, BoxLayout.Y_AXIS)
        );
        titleText.add(title);
    ...
    private JComponent activityMetric(
            String iconType,
            JLabel valueLabel,
            String text
    ) {
    ...
    private JComponent createTipsCard() {
        ...
        JLabel sub = label(
                "Make your feedback useful and trustworthy",
                12,
                Font.PLAIN,
                MUTED
        );
        ...
        tips.setLayout(
                new BoxLayout(tips, BoxLayout.Y_AXIS)
        );

        tips.add(tipLine(
                "Mention vehicle condition, cleanliness, and pickup experience."
        ));
        tips.add(Box.createVerticalStrut(6));

        tips.add(tipLine(
                "Keep your feedback honest, clear, and specific."
        ));
        tips.add(Box.createVerticalStrut(6));

        tips.add(tipLine(
                "Avoid sharing phone numbers or private information."
        ));
    ...
    private void submitReview() {
        ...
        if (message.isBlank()
                || message.equals("Write your review here...")) {
        ...
        String vehicle = String.valueOf(
                vehicleBox.getSelectedItem()
        );

        if (vehicle == null
                || vehicle.isBlank()
                || vehicle.startsWith("No rented vehicles")) {
    ...
    private void loadVehicleChoices() {
        ...
        java.util.LinkedHashSet<String> vehicleNames =
                new java.util.LinkedHashSet<>();

        for (RentalRepository.RentalRecord rental
                : rentalRepository.findByCustomerEmail(
                        customerEmail()
                )) {

            if (rental.vehicleName() != null
                    && !rental.vehicleName().isBlank()) {
    ...
    private void refreshReviewData() {
        ...
        double average = ratings.stream()
                .mapToInt(
                        RatingRepository.StoredRating::rating
                )
                .average()
                .orElse(0.0);
        ...
        int satisfaction = count == 0
                ? 0
                : (int) Math.round(
                        (positive * 100.0) / count
                );
        ...
        if (satisfactionValue != null) {
            satisfactionValue.setText(
                    satisfaction + "%"
            );
        }

        if (latestReviewValue != null) {
            latestReviewValue.setText(
                    count == 0
                            ? "—"
                            : formatReviewDate(
                                    ratings.get(0).createdAt()
                            )
            );
        }

        if (reviewsSharedValue != null) {
            reviewsSharedValue.setText(
                    String.valueOf(count)
            );
        }

        if (averageGivenValue != null) {
            averageGivenValue.setText(
                    String.format(
                            Locale.US,
                            "%.1f",
                            average
    ...
    private void refreshRecentReviews(
            List<RatingRepository.StoredRating> ratings
    ) {
        if (reviewsList == null
        ...
        if (ratings.isEmpty()) {
            ...
            for (int i = 0; i < limit; i++) {
                RatingRepository.StoredRating stored =
                        ratings.get(i);
    ...
    private ReviewText parseStoredComment(String comment) {
        ...
        String vehicle = value.substring(
                0,
                separator
        ).trim();
    ...
    private static final class ReviewActivityIcon
            extends JComponent {
    ...
    private static final class ReviewIcon
            extends JComponent {
    ...
    private static final class ReviewAvatar
            extends JComponent {
    ...
    private static final class StarRating
            extends JComponent {
